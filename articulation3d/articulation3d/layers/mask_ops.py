...
def paste_masks_in_image(masks, boxes, image_shape, threshold=0.5, nms=False):
    """
    ...
    for inds in chunks:
        ...
        if nms:
            nms_idx_mask = masks_chunk.max(dim=0, keepdim=True)[0] != masks_chunk
            masks_chunk[nms_idx_mask] = 0
        if threshold >= 0:
