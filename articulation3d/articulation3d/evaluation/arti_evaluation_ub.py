...
class ArtiEvaluator(COCOEvaluator):
    ...
    def process(self, inputs, outputs):
        ...
        for input, output in zip(inputs, outputs):
            ...
            if self.pointrend_detect(input['file_name']) and self.human_detect(input['file_name']):
                prediction['instances'] = []
                prediction['pred_tran_axis'] = []
                prediction['pred_rot_axis'] = []
                for ann in input['annotations']:
                    prediction['instances'].append({
                            'image_id': input['image_id'],
                            'category_id': ann['category_id'],
                            'bbox': BoxMode.convert(ann['bbox'], BoxMode.XYXY_ABS, BoxMode.XYWH_ABS),
                            'score': 1.0,
                        })

                    if ann['rot_axis'] is not None:
                        gt_rot_ao = axis_to_angle_offset([ann['rot_axis']], Boxes([ann['bbox']]).get_centers())[0].numpy()
                        prediction['pred_tran_axis'].append([0,0])
                        prediction['pred_rot_axis'].append(gt_rot_ao[:3])
                    elif ann['tran_axis'] is not None:
                        gt_tran_ao = axis_to_angle_offset([ann['tran_axis']], Boxes([ann['bbox']]).get_centers())[0].numpy()
                        prediction['pred_tran_axis'].append(gt_tran_ao[:2])
                        prediction['pred_rot_axis'].append([0,0,0])
                    else:
                        prediction['pred_tran_axis'].append([0,0])
                        prediction['pred_rot_axis'].append([0,0,0])
                    if len(prediction['pred_rot_axis'][-1]) == 4:
                        import pdb;pdb.set_trace()
                        pass
                ...
                prediction['pred_tran_axis'] = []
                prediction['pred_rot_axis'] = []
            prediction['pred_rot_axis'] = torch.Tensor(prediction['pred_rot_axis'])
            prediction['pred_tran_axis'] = torch.Tensor(prediction['pred_tran_axis'])
            self._predictions.append(prediction)

    def load_upper_bound_jsons(self):
        with open("/Pool1/users/jinlinyi/workspace/p-arti-videos/baselines/pointrend/detection.json",'r') as f:
            self.detection = json.load(f)
        with open("/Pool1/users/jinlinyi/workspace/p-arti-videos/baselines/EFT/maskiou.json",'r') as f:
            self.maskiou = json.load(f)
        # with open("/Pool1/users/jinlinyi/workspace/p-arti-videos/baselines/pointrend/detection_charades.json",'r') as f:
        #     self.detection = json.load(f)
        # with open("/Pool1/users/jinlinyi/workspace/p-arti-videos/baselines/EFT/maskiou_charades.json",'r') as f:
        #     self.maskiou = json.load(f)
    
    def pointrend_detect(self, file_name):
        try:
            det_list = self.detection[file_name]
        except:
            try:
                det_list = self.detection[file_name.replace('/z/', '/y/').replace('.png', '.jpg')]
            except:
                det_list = self.detection[file_name.replace('/z/', '/y/').replace('.png', '.jpg').replace('frames_hq_neg', 'frames_hq')]
    ...
    def human_detect(self, file_name):
        try:
            iou = self.maskiou[os.path.basename(file_name)]
        except:
            iou = self.maskiou[os.path.basename(file_name).replace('.png', '.jpg')]
        return iou > 0.5            
