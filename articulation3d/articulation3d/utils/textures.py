...
class Textures(object):
    def __init__(
        ...
        if map_files is not None:
            if not isinstance(map_files, list):
                ...
                raise ValueError(msg % type(map_file))
    ...
    def verts_uvs_list(self) -> Union[List[torch.Tensor], None]:
        ...
        return padded_to_list(self._verts_uvs_padded, split_size=self._num_verts_per_mesh)
    ...
    def extend(self, N: int) -> "Textures":
        ...
        if all(
            ...
            return Textures(
                verts_uvs=new_verts_uvs, faces_uvs=new_faces_uvs, map_files=new_map_files,
            )
