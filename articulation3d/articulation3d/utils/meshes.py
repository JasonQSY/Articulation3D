...
def join_meshes_as_batch(meshes: List[Meshes], include_textures: bool = True):
    """
    Source from https://pytorch3d.readthedocs.io/en/latest/_modules/pytorch3d/structures/meshes.html#Meshes
    Modified by Linyi Jin on combining textures
    ...
    if first.map_files() is not None:
        if any(mesh.textures.map_files() is None for mesh in meshes):
            raise ValueError("Inconsistent map_files in join_meshes_as_batch.")
        map_files = [m for mesh in meshes for m in mesh.textures.map_files()]
        kwargs["map_files"] = map_files
    elif any(mesh.textures.map_files() is not None for mesh in meshes):
        raise ValueError("Inconsistent map_files in join_meshes_as_batch.")
