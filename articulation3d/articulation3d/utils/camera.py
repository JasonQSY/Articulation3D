...
def write_cone_obj(positions, lookats, vertical, radius=0.02, output_file=None):
    """
    positions: np array (n, 3)
    output_file: string
    """
    ...
    for idx, (position, lookat) in enumerate(zip(positions, lookats)):
        ...
        cam_inds = []
        for k in range(len(edges)):
            cyl_verts, cyl_ind = create_arrow_mesh(radius, edges[k][0], edges[k][1])
        ...
        colors.append(color)
    if output_file:
        write_obj(verts, colors, indices, output_file, output_file.split('.')[0]+'.mtl')
    else:
...
def main():
    ...
    rot = np.array([[ -0.6863150, -0.1426361,  0.7131807],
    [0.1426361,  0.9351418,  0.3242912],
    [-0.7131807,  0.3242912, -0.6214568],])

    lookat = [
        [0,0,1],
        rot@np.array([0,0,1]).T
    ]
    vertical = [0,-1,0]

    write_cone_obj(positions, lookat, vertical, output_file='cone.obj')
