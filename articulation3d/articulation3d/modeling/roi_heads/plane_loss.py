...
class GeoConsistencyLoss(nn.Module):
    ...
    def pack_data(self, pred_instances1, pred_instances2, cam_poses, batched_inputs=None):
        ...
        if False:
            for x, cam_pose in zip(pred_instances1, cam_poses):
                ...
                planeparam_tmp = self.get_plane_params_in_global(planeparam_tmp, cam_pose)
            ...
            for x in pred_instances2:
                if has_instances_key:
                    ...
                    planeparam_tmp = x.pred_plane
                planeparam_tmp = planeparam_tmp*torch.FloatTensor([1,-1,-1]).to(self.device)
        ...
        if batched_inputs is not None:
            for x1, x2, x in zip(pred_instances1, pred_instances2, batched_inputs):
                ...
                gt_corr = x['gt_corrs']
                gt_corr_m = torch.zeros([len(planeparam1[0]), len(planeparam2[0])], dtype=torch.bool)
                for corr in gt_corr:
        ...
        return planeparam1, planeparam2, gt_corr_ms, numPlanes1, numPlanes2, 

    def forward(self, batched_inputs, pred_instances1, pred_instances2, cam_poses, loss_weight):
        planeparam1, planeparam2, gt_corr_ms, numPlanes1, numPlanes2,  = self.pack_data(
            pred_instances1, pred_instances2, cam_poses, batched_inputs)
    ...
    def inference(self, pred_instances1, pred_instances2, cam_poses, distance='l2'):
        ...
        if distance == 'l2':
            ...
            geo_distance_matrix['offset'][nTn<0] = torch.abs(offset1+offset2.transpose(1,2))[nTn<0]
            geo_distance_matrix['normal'] = torch.acos(torch.clamp(torch.abs(nTn), -1, 1)) # in rad
        else:
    ...
    def get_plane_params_in_global(self, planes, camera_info):
        ...
        end = torch.mm(torch.FloatTensor(quaternion.as_rotation_matrix(rot)).to(self.device), (end).T).T + tran #cam2world
        a = end
        b = end-start
